...
public class RuleNet {
	...
	private ArrayList<ArrayList<RuleNode>> targetNodeLists; // each element is list of nodes which corresponding fact
	...
	/**
	 * Initialise all the RuleNodes to contain a single token: all antecedents,
	 * empty context
	 */
	...
	/**
	 * add a fact and make all the deductions by calling self.propagate
	 */
	...
	/**
	 * drive the propagation of facts take next fact from factsToPropagate take the
	 * list of nodes to propagate it to from targetNodeLists do this using the
	 * node's propagate method if this results in deductions, then for each
	 * deduction print it out add it to factsToPropagate add the rulenode's
	 * successors list to targetNodeLists continue until no more facts to propagate
	 */

	public void propagate() { // drive the propagation of facts
		while (!factsToPropagate.isEmpty()) {
			String nextFact = factsToPropagate.get(factsToPropagate.size() - 1);
			factsToPropagate.remove(factsToPropagate.size() - 1);
			ArrayList<RuleNode> nodeList = targetNodeLists.get(targetNodeLists.size() - 1);
			targetNodeLists.remove(targetNodeLists.size() - 1);
			ArrayList<String> returnedDeductions;
			for (RuleNode node : nodeList) {
				if (node.propagate(nextFact)) {
					returnedDeductions = node.getDeductions();
					for (String deduction : returnedDeductions) {
						System.out.println("Deduction Made: " + deduction);
						factsToPropagate.add(deduction);
						targetNodeLists.add(node.getSuccessors());
					}
				}
			}
		}
